...
import {
  getDriverPlaceHolderData,
  getRaces,
  getTeamPlaceHolderData,
  getTracks,
...
async function seedDrivers(teams: { id: string; name: string }[]) {
  ...
  for (const driver of drivers) {
    const driver_full_name = `${driver.first_name} ${driver.last_name}`;
    const image_url = driver_full_name
      .toLowerCase()
      .replaceAll(" ", "-")
      .concat(".png");
    console.log(driver);
    const result = await sql`
            INSERT INTO drivers (first_name, last_name, team_id, number, image_url)
            VALUES (${driver.first_name}, ${driver.last_name}, ${driver.team_id}, ${driver.number}, ${image_url})
